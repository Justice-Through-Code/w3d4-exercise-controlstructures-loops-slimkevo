...
def favorite_activities():
    ...
    favorite_activities = []
    for day in days:
        favorite_things = input(f"What is your favorite thing to do on {day}? ")
    ...
    for i in range(len(days)):
        day = days[i]
        favorite_activity = favorite_activities[i]
        print(f'On {day}s, your favorite activity is to {favorite_activity}.')
...
def temp_by_day_continuous():
    ...
    outside = int(input(f'What is the temperature outside?'))
    while outside < 65:
